let materi_json = [
  {
    "id": "MAT-01",
    "judul": "Sikap Siap (Ready Position)",
    "model3D": "ready_idle-compressed.glb",
    "deskripsi": "Sikap awal bersiap sebelum melakukan gerakan atau menerima bola.",
    "langkah": [
      "Berdiri tegak menghadap net.",
      "Kedua kaki dibuka selebar bahu.",
      "Lutut sedikit ditekuk.",
      "Berat badan bertumpu pada ujung kaki.",
      "Pegang raket di depan badan dengan kedua tangan.",
      "Pandangan fokus ke arah bola dan lawan.",
      "Siap bergerak ke segala arah."
    ]
  },
  {
    "id": "MAT-02",
    "judul": "Footwork (Gerakan Kaki)",
    "model3D": "footwork-compressed.glb",
    "deskripsi": "Teknik pergerakan kaki untuk memposisikan tubuh dengan ideal.",
    "langkah": [
      "Berdiri dalam posisi siap.",
      "Bergerak ringan dengan langkah kecil.",
      "Gunakan ujung kaki agar gerakan cepat.",
      "Melangkah ke kanan, kiri, depan, atau belakang mengikuti arah bola.",
      "Jaga keseimbangan tubuh saat bergerak.",
      "Tempatkan tubuh pada posisi ideal sebelum memukul bola.",
      "Setelah memukul, segera kembali ke posisi siap."
    ]
  },
  {
    "id": "MAT-03",
    "judul": "Forehand",
    "model3D": "forehand.glb",
    "deskripsi": "Pukulan dasar dari sisi dominan tubuh.",
    "langkah": [
      "Berdiri dalam posisi siap, lutut sedikit ditekuk.",
      "Pegang raket dengan grip forehand.",
      "Saat bola datang, putar bahu dan tarik raket ke belakang.",
      "Langkahkan kaki depan ke arah bola.",
      "Ayunkan raket dari belakang ke depan.",
      "Pukul bola di samping badan setinggi pinggang.",
      "Lanjutkan ayunan ke depan atas (follow through).",
      "Kembali ke posisi siap."
    ]
  },
  {
    "id": "MAT-04",
    "judul": "Backhand",
    "model3D": "backhand.glb",
    "deskripsi": "Pukulan dasar dari sisi non-dominan tubuh.",
    "langkah": [
      "Berdiri dalam posisi siap.",
      "Pegang raket dengan grip backhand (bisa satu tangan atau dua tangan).",
      "Putar badan menyamping dan tarik raket ke belakang.",
      "Langkahkan kaki depan ke arah bola.",
      "Ayunkan raket dari belakang ke depan.",
      "Pukul bola di depan badan.",
      "Lanjutkan ayunan ke arah sasaran.",
      "Kembali ke posisi siap."
    ]
  },
  {
    "id": "MAT-05",
    "judul": "Servis",
    "model3D": "new-c.glb",
    "deskripsi": "Pukulan pertama untuk memulai permainan.",
    "langkah": [
      "Berdiri di belakang garis baseline.",
      "Pegang raket dengan tangan dominan, bola di tangan satunya.",
      "Posisi badan menyamping ke arah net.",
      "Lambungkan bola ke atas lurus.",
      "Tarik raket ke belakang kepala.",
      "Saat bola turun, ayunkan raket ke atas dan pukul bola.",
      "Pukul bola sekuat dan setepat mungkin ke kotak servis lawan.",
      "Lanjutkan gerakan tubuh ke depan masuk lapangan."
    ]
  },
  {
    "id": "MAT-06",
    "judul": "Volley Forehand",
    "model3D": "volley_forehand-compressed.glb",
    "deskripsi": "Pukulan forehand di udara sebelum bola memantul di lapangan.",
    "langkah": [
      "Berdiri dekat net dengan posisi siap.",
      "Pegang raket di depan badan.",
      "Saat bola datang ke sisi forehand, putar sedikit bahu.",
      "Langkahkan kaki depan ke arah bola.",
      "Dorong raket ke depan tanpa ayunan besar.",
      "Pukul bola sebelum memantul.",
      "Arahkan bola ke area kosong lawan.",
      "Kembali ke posisi siap."
    ]
  },
  {
    "id": "MAT-07",
    "judul": "Volley Backhand",
    "model3D": "volley_backhand-compressed.glb",
    "deskripsi": "Pukulan backhand di udara sebelum bola memantul.",
    "langkah": [
      "Berdiri dekat net dalam posisi siap.",
      "Pegang raket di depan badan.",
      "Putar bahu ke sisi backhand.",
      "Langkahkan kaki depan ke arah bola.",
      "Dorong raket ke depan pendek dan stabil.",
      "Pukul bola sebelum memantul.",
      "Arahkan bola ke sisi lawan.",
      "Kembali ke posisi siap."
    ]
  }
]
